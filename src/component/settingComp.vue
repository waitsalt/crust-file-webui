...
import { ref } from 'vue';
import { BiExport, BiImport, BiPlus, BiTrash } from 'vue-icons-plus/bi';
...
const exportSettings = () => {
    const data = JSON.stringify(settingStore.$state);
    const blob = new Blob([data], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `crust-settings-${new Date().toISOString().slice(0, 10)}.json`;
    a.click();
